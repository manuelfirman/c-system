...
bool Fecha::leerDeDisco(FILE* file) {
    if(fread(&dia, sizeof(dia), 1, file) != 1) return false;
    if(fread(&mes, sizeof(mes), 1, file) != 1) return false;
    if(fread(&anio, sizeof(anio), 1, file) != 1) return false;

    return true;
}

bool Fecha::grabarEnDisco(FILE* file) const {
    if(fwrite(&dia, sizeof(dia), 1, file) != 1) return false;
    if(fwrite(&mes, sizeof(mes), 1, file) != 1) return false;
    if(fwrite(&anio, sizeof(anio), 1, file) != 1) return false;
